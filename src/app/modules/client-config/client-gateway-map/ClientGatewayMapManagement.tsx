...
import { deleteClientGatewayMap, getListClientGatewayMap, resetDept } from './ClientGatewayMapManagement.reducer.ts';
...
import EditClientGatewayMap from './edit/EditClientGatewayMapManegement.tsx';
import { checkSuccessDispatch } from '@/app/shared/util/global-function';
import { BANK_CODE_STATUS } from '@/app/config/constant/enum';
const ClientGatewayMap = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.clientGatewayMapManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.clientGatewayMapManagementReducer.loadingDelete);
  const listClientGatewayMap = useAppSelector(state => state.clientGatewayMapManagementReducer.listClientGatewayMap);
  ...
  const _onDelete = async () => {
    ...
    dispatch(deleteClientGatewayMap(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
  ...
  const _onSearchAction = async () => {
    ...
    dispatch(getListClientGatewayMap(payload));
  };
      ...
      <div className="page-content-template">
        <div className="page-search-template">
          <Row style={{ marginBottom: 18 }} align="middle">
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">clientIdCms</label>
            </Col>
        ...
        <div className="page-table-template">
          ...
          <Spin spinning={loading} tip={i18next.t('getData')}>
            <div className="table-data">
              <DataGrid
                id="gridContainer"
                dataSource={listClientGatewayMap}
                showBorders={true}
                keyExpr="idx"
                ref={dataGridRef}
              ...
              >
                ...
                />
                <Column
                  dataField="openApiAlgorithmApplyYn"
                  alignment="center"
                  allowFiltering={false}
                  allowSorting={true}
                  caption="Status algorithm"
                  dataType="string"
                  width={120}
                  cellRender={row => {
                    if (!row.data.openApiAlgorithmApplyYn) {
                      return <>{row.data.openApiAlgorithmApplyYn}</>;
                    }
                    const requestStatusData = listActionType.find(obj => obj.val === row.data.openApiAlgorithmApplyYn);
...
export default ClientGatewayMap;
