...
import { deleteClientCallBackManagement, getListClientCallBackManagement, resetDept } from './ClientCallBackManagement.reducer.ts';
...
import EditClientCallBackManagement from './edit/EditClientCallBackManagement.tsx';
import { checkSuccessDispatch } from '@/app/shared/util/global-function';
import { BANK_CODE_STATUS } from '@/app/config/constant/enum';
const ClientCallBackManagement = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.clientCallbackManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.clientCallbackManagementReducer.loadingDelete);
  const listClientCallBackManagement = useAppSelector(state => state.clientCallbackManagementReducer.listClientCallBackManagement);
  ...
  const _onDelete = async () => {
    ...
    dispatch(deleteClientCallBackManagement(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
  ...
  const _onSearchAction = async () => {
    ...
    const payload = createCommonIParamsDuong({ clientIdGw, cbType, url, useYn, pageRequestDto });
    dispatch(getListClientCallBackManagement(payload));
  };
        ...
        <div className="page-table-template">
          ...
          <Spin spinning={loading} tip={i18next.t('getData')}>
            <div className="table-data">
              <DataGrid
                id="gridContainer"
                dataSource={listClientCallBackManagement}
                showBorders={true}
                keyExpr="idx"
                ref={dataGridRef}
              ...
              >
                ...
                <Column
                  ...
                  width={180}
                />
                <Column dataField="url" alignment="left" allowFiltering={false} allowSorting={true} caption="Url" dataType="string" />
                <Column
                  dataField="useYn"
                  alignment="center"
                  allowFiltering={false}
...
export default ClientCallBackManagement;
