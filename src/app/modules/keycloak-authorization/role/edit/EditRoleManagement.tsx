...
import { insertRole, updateRole } from '../RoleManagement.reducer';
const EditRoleManagement = ({ isEdit, onSearch, selected, onChangeFormAdd }) => {
    const dispatch = useAppDispatch();

    const { loadingAdd, validateError, loadingUpdate } = useAppSelector(state => state.roleManagementReducer);
    ...
    const _onSubmit = async (values: any) => {
        const payload = createCommonIParamsDuong({ id: values.id, roleName: values.name.trim(), desc: values.description.trim() });
        if (isEdit) {
            dispatch(updateRole(payload)).then(res => {
        ...
        dispatch(insertRole(payload)).then(res => {
            if (checkSuccessDispatch(res)) {
                _onSuccess();
            }
        });
    ...
    return (
        <div className="insert edit-department">
            <div className="heading">
                <h3>{isEdit ? 'edit role Management' : 'role Management'}</h3>
            </div>
            ...
            <div className="content">
                ...
                <Form form={formRegis} onFinish={_onSubmit} className="custom-form edit-department__form">
                    <Row align="middle" style={{ marginBottom: 12 }}>
                        ...
                        <Col xs={10} md={3}>
                            <span className="cms-search-label">Role description</span>
                            <span className="cms-required-field"> *</span>
                        </Col>
                        <Col xs={14} md={5}>
