...
import { deleteResource, getListResource, resetDept, selectResource } from '../resource/ResourceManagement.reducer.ts';
import 'devextreme/dist/css/dx.common.css';
import 'devextreme/dist/css/dx.light.css';

import { cellIndexRender } from '../../common/JSXFunction';
import '../../../shared/layout/content-task.scss';
import EditResource from './edit/EditResourceManagement.tsx';
import { checkSuccessDispatch } from '@/app/shared/util/global-function';

const Resource = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.resourceKeycloakManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.resourceKeycloakManagementReducer.loadingDelete);
  const listResource = useAppSelector(state => state.resourceKeycloakManagementReducer.listResource);
  ...
  const _handleChangeResourceName = e => {
  ...
  const _onDelete = async () => {
    ...
    listSelected.forEach(item => (listDelete = item._id));

    dispatch(deleteResource(createCommonIParamsListDuong({ resourceId: listDelete }))).then(res => {
  ...
  const _onSearchAction = async () => {
    ...
    dispatch(getListResource(payload));
  };
          ...
          <Row style={{ marginBottom: 18 }} align="middle">
            ...
            <Col xl={5} xxl={4}>
              <Input
                ...
                value={resourceName}
                onChange={_handleChangeResourceName}
                maxLength={80}
                onKeyPress={_onEnter}
