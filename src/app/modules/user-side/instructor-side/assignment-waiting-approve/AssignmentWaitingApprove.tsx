...
import { deleteAssigmentWaitingApprove, getListAssigmentWaitingApprove, resetDept, sendRequestAssignmentStudent} from './AssigmentWaitingApprove.reducer';

import 'devextreme/dist/css/dx.common.css';
import 'devextreme/dist/css/dx.light.css';
import '../../../../shared/layout/content-task.scss';
// import EditAssignmentStudentRegister from './edit/EditAssigmentWaitingApprove';
...
const AssigmentWaitingApprove = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.AssigmentWaitingApprove.loading);
  const loadingDelete = useAppSelector(state => state.AssigmentWaitingApprove.loadingDelete);
  const listAssigmentWaitingApprove = useAppSelector(state => state.AssigmentWaitingApprove.listAssigmentWaitingApprove);
  const totalRecord = useAppSelector(state => state.AssigmentWaitingApprove.totalRecord);
  const [AssigmentWaitingApproveId, setAssigmentWaitingApproveId] = useState('');
  const [AssigmentWaitingApproveName, setAssigmentWaitingApproveName] = useState('');
  ...
  const _handleChangeAssigmentWaitingApproveId = e => {
    const { value } = e.target;
    setAssigmentWaitingApproveId(value);
  };
  ...
  const _handleChangeAssigmentWaitingApproveName = e => {
    const { value } = e.target;
    setAssigmentWaitingApproveName(value);
  };
  ...
  const _onChangeSendRequest = value => {
    ...
    dispatch(sendRequestAssignmentStudent(createCommonIParamsDuong({ requestId }))).then(res => {
      if (checkSuccessDispatch(res)) {
        onScrollToTop();
        _onSearchAssigmentWaitingApprove(pager.pageNum, pager.pageSize);
      }
    });
  };
  ...
  const _onChangePagination = (page, pageSize) => {
    ...
    _onSearchAssigmentWaitingApprove(page, pageSize);
  };

    const _onSearchAssigmentWaitingApprove = async (pageNum, pageSize) => {
      setListSelected([]);
    ...
    const payload = createCommonIParamsDuong({ AssigmentWaitingApproveId, AssigmentWaitingApproveName, fromDate, toDate, status, pageRequestDto });
    dispatch(getListAssigmentWaitingApprove(payload));
    };
  
    ///For search button
    const _onSearchAssigmentWaitingApprove2 = async () => {
    setListSelected([]);
    dataGridRef.current.instance.clearSelection();
    let pageRequestDto = {
      pageNum: 1,
    ...
    const payload = createCommonIParamsDuong({ AssigmentWaitingApproveId, AssigmentWaitingApproveName, instructorId, fromDate, toDate, status, pageRequestDto });
    dispatch(getListAssigmentWaitingApprove(payload));
    };

  const onChangeDate = value => {
    const AssigmentWaitingApproveFromDt = value && value.length === 2 ? moment(value[0]).format(APP_DATE_FORMAT) : '';
    const AssigmentWaitingApproveToDt = value && value.length === 2 ? moment(value[1]).format(APP_DATE_FORMAT) : '';
    setFromDate(AssigmentWaitingApproveFromDt);
    setToDate(AssigmentWaitingApproveToDt);
    };
  ...
  const _onDeleteAssigmentWaitingApprove = () => {
    Modal.confirm({
      ...
      async onOk() {
        await _onDelete();
        await _onSearchAssigmentWaitingApprove(pager.pageNum, pager.pageSize);
      },
    });
  };
  ...
  const _onDelete = async () => {
    const listDelete = [];
    listSelected.forEach(item => listDelete.push(item.AssigmentWaitingApproveId));
    dispatch(deleteAssigmentWaitingApprove(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
      if (checkSuccessDispatch(res)) {
        onScrollToTop();
        _onSearchAssigmentWaitingApprove(pager.pageNum, pager.pageSize);
      }
    ...
    <div className="page-content-template department-management">
      ...
      <div className="page-content-template">
        <div className="page-search-template">
          <Row style={{ marginBottom: 18 }} align="middle">
            ...
            <Col xl={5} xxl={4}>
              <Input
                ...
                value={AssigmentWaitingApproveId}
                onChange={_handleChangeAssigmentWaitingApproveId}
                maxLength={80}
                onKeyPress={_onEnter}
            ...
            <Col xl={5} xxl={4}>
              <Input
                className="cms-form-control"
                value={AssigmentWaitingApproveName}
                onChange={_handleChangeAssigmentWaitingApproveId}
                maxLength={80}
                onKeyPress={_onEnter}
                placeholder="Nhập vào đi bạn"
          ...
          <Row align="middle">
            ...
            <Col className="form-btn-search">
              <Button icon={<SearchOutlined />} loading={loading} className="button-search" onClick={_onSearchAssigmentWaitingApprove2}>
                Tìm kiếm
              </Button>
            </Col>
          </Row>
        ...
        <div className="page-table-template">
          <div className="table-actions">
            ...
            <Button
              ...
              icon={<DeleteOutlined />}
              onClick={_onDeleteAssigmentWaitingApprove}
            >
          ...
          <Spin spinning={loading} tip={i18next.t('getData')}>
            <div className="table-data">
              <DataGrid
                id="gridContainer"
                dataSource={listAssigmentWaitingApprove}
                showBorders={true}
        ...
        <EditAssignmentStudentRegister
          ...
          onSearch={_onSearchAssigmentWaitingApprove}
...
export default AssigmentWaitingApprove;
