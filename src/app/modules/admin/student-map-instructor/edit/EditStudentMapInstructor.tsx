...
import { insertStudentMapInstructor, updateStudentMapInstructor, selectStudentMapInstructor } from '../StudentMapInstructor.reducer';
const EditStudentMapInstructor = ({ isEdit, onSearch, selected, onChangeFormAdd, listInstructor, listStudent }) => {
  const dispatch = useAppDispatch();
  const { loadingAdd, validateError, loadingUpdate } = useAppSelector(state => state.studentMapInstructor);
  ...
  const _onSubmit = async (values: any) => {
    if (isEdit) {
      _onUpdateStudentMapInstructor(values);
      return;
    } else {
      const payload = createCommonIParamsDuong({
       instructorId: values.instructorId, studentId: values.studentId
      });
      dispatch(insertStudentMapInstructor(payload)).then(res => {
  ...
  const _onUpdateStudentMapInstructor = (values: any) => {
    const payload = createCommonIParamsDuong({
      studentMapInstructorId: values.studentMapInstructorId , instructorId: values.instructorId, studentId: values.studentId
    });
    dispatch(updateStudentMapInstructor(payload)).then(res => {
      if (checkSuccessDispatch(res)) {
        _onSuccess();
      }
    });
...
export default EditStudentMapInstructor;