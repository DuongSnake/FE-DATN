...
import { deleteGatewayApi, getListGatewayApi, resetDept } from './GatewayApiManagement.reducer.ts';
...
// import EditGatewayApi from './edit/EditGatewayApiManagement.tsx';
import { checkSuccessDispatch } from '@/app/shared/util/global-function';
import { BANK_CODE_STATUS } from '@/app/config/constant/enum';
import EditGatewayApiManagement from './edit/EditGatewayApiMangement.tsx';

const GateWayApi = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.gatewayApiManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.gatewayApiManagementReducer.loadingDelete);
  const listGetwayApi = useAppSelector(state => state.gatewayApiManagementReducer.listGatewayApi);
  const [apiId, setApiId] = useState('');
  const [apiNm, setApiNm] = useState('');
  ...
  const _onDelete = async () => {
    ...
    listSelected.forEach(item => listDelete.push(item.apiId));

    dispatch(deleteGatewayApi(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
  ...
  const _onSearchAction = async () => {
    ...
    const payload = createCommonIParamsDuong({ apiId, apiNm, status, pageRequestDto });
    dispatch(getListGatewayApi(payload));
  };
      ...
      <div className="page-content-template">
        <div className="page-search-template">
          <Row style={{ marginBottom: 18 }} align="middle">
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">Api Id</label>
            </Col>
            ...
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">Api name</label>
            </Col>
