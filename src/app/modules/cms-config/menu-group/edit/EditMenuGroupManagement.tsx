...
import { insertMenuGroup, updateMenuGroup } from '../MenuGroupManagement.reducer';
const EditMenuGroupManagement = ({ isEdit, onSearch, selected, onChangeFormAdd }) => {
    const dispatch = useAppDispatch();

    const { loadingAdd, validateError, loadingUpdate } = useAppSelector(state => state.menuGroupManagementReducer);
    ...
    const _onSubmit = async (values: any) => {
        const payload = createCommonIParamsDuong({ menuId: values.menuId.trim(), menuNm: values.menuNm.trim(), sys: values.sys.trim() });
        if (isEdit) {
            dispatch(updateMenuGroup(payload)).then(res => {
        ...
        dispatch(insertMenuGroup(payload)).then(res => {
            if (checkSuccessDispatch(res)) {
                _onSuccess();
            }
        });
    ...
    return (
        <div className="insert edit-department">
            <div className="heading">
                <h3>{isEdit ? 'edit menu Group Management' : 'register menu Group Management'}</h3>
            </div>
...
export default EditMenuGroupManagement;