...
import { deleteMenuGroup, getListMenuGroup, resetDept } from './MenuGroupManagement.reducer.ts';
...
import EditMenuGroup from './edit/EditMenuGroupManagement.tsx';
...
const MenuGroup = () => {
  ...
  const loading = useAppSelector(state => state.menuGroupManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.menuGroupManagementReducer.loadingDelete);
  const listMenuGroup = useAppSelector(state => state.menuGroupManagementReducer.listMenuGroup);
  const [menuId, setMenuGroupId] = useState('');
  const [menuNm, setMenuGroupNm] = useState('');
  ...
  const _handleChangeMenuGroupId = e => {
  ...
  const _handleChangeMenuGroupNm = e => {
    const { value } = e.target;
    setMenuGroupNm(value);
  };
  ...
  const _onDelete = async () => {
    ...
    dispatch(deleteMenuGroup(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
  ...
  const _onSearchAction = async () => {
    ...
    dispatch(getListMenuGroup(payload));
  };
          ...
          <Row style={{ marginBottom: 18 }} align="middle">
            ...
            <Col xl={5} xxl={4}>
              <Input
                ...
                value={menuId}
                onChange={_handleChangeMenuGroupId}
                maxLength={80}
                onKeyPress={_onEnter}
            ...
            <Col xl={5} xxl={4}>
              <Input
                className="cms-form-control"
                value={menuNm}
                onChange={_handleChangeMenuGroupNm}
                maxLength={60}
                onKeyPress={_onEnter}
                placeholder={i18next.t('cms-common.please-input-infomation')}
              />
