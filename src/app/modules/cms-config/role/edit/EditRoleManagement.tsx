...
const EditRoleManagement = ({ isEdit, onSearch, selected, onChangeFormAdd }) => {
    ...
    const _onSubmit = async (values: any) => {
        const payload = createCommonIParamsDuong({ roleCd: values.roleCd.trim(), roleNm: values.roleNm.trim(), sys: values.sys.trim() });
        if (isEdit) {
    ...
    return (
        <div className="insert edit-department">
            <div className="heading">
                <h3>{isEdit ? 'edit role Management' : 'register role Management'}</h3>
            </div>
