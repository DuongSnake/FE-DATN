...
import { deleteMenu, getListMenu, resetDept } from './MenuManagement.reducer.ts';
...
import EditMenu from './edit/EditMenuManagement.tsx';
import { checkSuccessDispatch } from '@/app/shared/util/global-function';
import { BANK_CODE_STATUS } from '@/app/config/constant/enum';
import { getListMenuGroup } from '../menu-group/MenuGroupManagement.reducer.ts';
const Menu = () => {
  const dispatch = useAppDispatch();
  const loading = useAppSelector(state => state.menuManagementReducer.loading);
  const loadingDelete = useAppSelector(state => state.menuManagementReducer.loadingDelete);
  const listMenu = useAppSelector(state => state.menuManagementReducer.listMenu);
  ...
  const _handleChangeMenuNm = e => {
    const { value } = e.target;
    setMenuNm(value);
  };

  const _handleChangeSys = e => {
    const { value } = e.target;
    setSys(value);
  };

  const _handleChangeParent = e => {
    const { value } = e.target;
    setParent(value);
  };
  ...
  const onSelectionChanged = async () => {
    ...
    const payload = createCommonIParamsDuong({ menuId: null, menuNm: null, sys: null, parent, status, pageRequestDto });
    dispatch(getListMenuGroup(payload)).then(res => {
      if (checkSuccessDispatch(res)) {
        const objectResponse: IParamCommonDuong = res.payload;
        setListParent(objectResponse.data.listData);
      }
    });
  };
  ...
  const _onDelete = async () => {
    ...
    listSelected.forEach(item => listDelete.push(item.menuId));

    dispatch(deleteMenu(createCommonIParamsListDuong({ listData: listDelete }))).then(res => {
      if (checkSuccessDispatch(res)) {
        onScrollToTop();
        _onSearchAction();
      }
  ...
  const _onSearchAction = async () => {
    ...
    const payload = createCommonIParamsDuong({ menuId, menuNm, sys, parent, status, pageRequestDto });
      ...
      <div className="page-content-template">
        <div className="page-search-template">
          <Row style={{ marginBottom: 18 }} align="middle">
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">Menu Id</label>
            </Col>
            ...
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">Menu name</label>
            </Col>
            ...
            <Col xl={3} xxl={3}>
              <label className="cms-search-label label-padding-left">System code</label>
            </Col>
